...
execute if score ready_mode sgn1.settings matches 2 run item replace entity @s[tag=!sgn1.ready] hotbar.2 with barrier{hotbar:1b,clear:1b,display:{"Name":'[{"text":"Не готов ","color":"red","italic":false},{"text":"[X]","color":"red","italic":true}]'}}
