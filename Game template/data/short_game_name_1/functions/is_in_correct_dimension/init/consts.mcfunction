

# scoreboard players set rejoin_threshold sgn1.settings 6000
scoreboard players set active sgn1.settings 1
scoreboard players set teams sgn1.settings 0
scoreboard players set ready_mode sgn1.settings 0

data modify storage core:errors short_game_name_1.log set value []


scoreboard players set max.red sgn1.settings 4
scoreboard players set max.green sgn1.settings 4
scoreboard players set max.blue sgn1.settings 4
scoreboard players set max.yellow sgn1.settings 4
scoreboard players set max.dark_red sgn1.settings 4
scoreboard players set max.dark_green sgn1.settings 4
scoreboard players set max.dark_blue sgn1.settings 4
scoreboard players set max.gold sgn1.settings 4